 ...
 *
 * GET    — list attachments for a bill
...
export const GET = withAuth(
  async (request, { params }) => {
    try {
      ...
      console.error("[GET /api/v1/bills/[id]/attachments] error:", err);
      return NextResponse.json(
...
export const POST = withAuth(
  async (request, { params }) => {
    try {
      ...
      let fileUrl;
      try {
        fileUrl = await uploadThumbnail(file, "bills");
      } catch (err) {
        console.error("[POST /api/v1/bills/[id]/attachments] upload error:", err);
        return NextResponse.json(
      ...
      try {
        ...
        console.error("[POST /api/v1/bills/[id]/attachments] DB error:", err);
      ...
      console.error("[POST /api/v1/bills/[id]/attachments] error:", err);
      return NextResponse.json(
  ...
  async (request, { params }) => {
    try {
      ...
      const attachment = await prisma.wehowareAttachment.findFirst({
        where: { id: attachmentId, billId: id, clientId },
      });
      if (!attachment) {
        return NextResponse.json(
      ...
      await deleteThumbnailByUrl(attachment.fileUrl).catch((err) => {
        console.warn("[DELETE /api/v1/bills/[id]/attachments] file cleanup failed:", err?.message);
      });
      ...
      console.error("[DELETE /api/v1/bills/[id]/attachments] error:", err);
      return NextResponse.json(
