/**
 * /api/v1/services/[id]/faqs/[faqId]
 *
 * Admin API for a single service FAQ.
 * PUT    — update
 * DELETE — remove
 */
...
export const PUT = withAuth(
  async (request, { params }) => {
    try {
      ...
      const { id: serviceId, faqId } = await params;
      ...
      const existing = await prisma.wehowareServiceFaq.findFirst({
        where: { id: faqId, serviceId },
      });
      if (!existing) {
        return NextResponse.json(
          { error: "FAQ not found" },
      ...
      const updated = await prisma.wehowareServiceFaq.update({
        where: { id: faqId },
        data: {
      ...
      console.error("[PUT /api/v1/services/[id]/faqs/[faqId]] error:", err);
      return NextResponse.json(
...
export const DELETE = withAuth(
  async (request, { params }) => {
    try {
      ...
      await prisma.wehowareServiceFaq.delete({ where: { id: faqId } });
      return NextResponse.json({ success: true });
    } catch (err) {
      console.error("[DELETE /api/v1/services/[id]/faqs/[faqId]] error:", err);
      return NextResponse.json(
