 ...
 *
 * Bulk operations on services within the active client context.
 *
...
export const POST = withAuth(
  async (request) => {
    try {
      ...
      if (action === "delete") {
        const { count } = await prisma.wehowareService.deleteMany({ where });
        return NextResponse.json({ deleted: count, updated: 0 });
      }

      const { count } = await prisma.wehowareService.updateMany({
        where,
        data: {
      ...
      console.error("[POST /api/v1/services/bulk] error:", err);
