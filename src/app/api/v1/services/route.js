...
// GET services with client filtering based on user role
    ...
    if (request.user.role === 'client') {
      ...
      if (!clientId) {
        return NextResponse.json(
          { error: 'Client ID is required for employee users' },
          { status: 400 }
        );
      }
    }
...
export const POST = withAuth(createService, { allowedRoles: ['client', 'employee', 'admin'] });

// For dynamic route parameters, Next.js requires these to be in a separate file
// Create [id]/route.js files for these handlers
