...
export default function EditBlogPage({ params }) {
  ...
  // Fetch blog post and categories on mount
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsFetching(true);
        await Promise.all([fetchBlogPost(), fetchCategories()]);
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // If the blog is being published for the first time, set published_at to now
      ...
      setErrorMessage(error.message || "Failed to update blog post");
      setErrorDialogOpen(true);
    } finally {
      setIsLoading(false);
    }
            ...
            <div className="space-y-4">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Update the main details for your blog post
                  </CardDescription>
                </CardHeader>
