 ...
 * GET / POST for inventory categories
 */
...
async function generateSlug(prisma, name, clientId) {
  const base = String(name).toLowerCase().replace(/\s+/g, "-").replace(/[^\w-]+/g, "").replace(/--+/g, "-").replace(/^-+|-+$/g, "") || "category";
  let candidate = base;
  let counter = 1;
  while (counter < 100) {
    const hit = await prisma.wehowareInventoryCategory.findFirst({ where: { clientId, slug: candidate }, select: { id: true } });
...
export const GET = withAuth(async (request) => {
  try {
    ...
    const categories = await prisma.wehowareInventoryCategory.findMany({ where, orderBy: { name: "asc" } });
    return NextResponse.json({ data: categories.map(serialize) });
  } catch (err) {
    console.error("[GET /api/v1/inventory/categories]", err);
...
export const POST = withAuth(async (request) => {
  try {
    ...
    const slug = await generateSlug(prisma, body.name, clientId);
    const category = await prisma.wehowareInventoryCategory.create({
      data: {
        clientId,
        name: body.name,
        slug,
    ...
    console.error("[POST /api/v1/inventory/categories]", err);
