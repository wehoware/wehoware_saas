 ...
 * PUT / DELETE for a single inventory category
 */
...
async function load(prisma, user, id) {
  ...
  const cat = await prisma.wehowareInventoryCategory.findFirst({ where: { id, clientId } });
...
export const PUT = withAuth(async (request, { params }) => {
  try {
    ...
    const updated = await prisma.wehowareInventoryCategory.update({ where: { id }, data });
    ...
    console.error("[PUT /api/v1/inventory/categories/[id]]", err);
...
export const DELETE = withAuth(async (request, { params }) => {
  try {
    ...
    const itemCount = await prisma.wehowareInventoryItem.count({ where: { categoryId: id } });
    if (itemCount > 0) {
      return NextResponse.json(
        { error: `Cannot delete category — ${itemCount} inventory item(s) are still assigned to it. Please reassign or remove those items first.` },
        { status: 409 }
      );
    }

    await prisma.wehowareInventoryCategory.delete({ where: { id } });
    return NextResponse.json({ success: true });
  } catch (err) {
    console.error("[DELETE /api/v1/inventory/categories/[id]]", err);
