                ...
                <CardContent className="space-y-4">
                  {images.map((img, index) => (
                    <div key={`img-${index}`} className="flex items-center gap-2">
                      ...
                      <Input placeholder="Image URL" value={img.url} onChange={(e) => handleImageChange(index, "url", e.target.value)} className="flex-1" disabled={img.uploading || isBatchUploading} />
                      <Input placeholder="Alt text (optional)" value={img.alt} onChange={(e) => handleImageChange(index, "alt", e.target.value)} className="flex-1" disabled={isBatchUploading} />
                      <Button type="button" variant="outline" size="icon" disabled={img.uploading || isBatchUploading} onClick={() => {
                      ...
                      <Button type="button" variant="ghost" size="icon" onClick={() => handleRemoveImage(index)} disabled={isBatchUploading} title="Remove">
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
                  ))}
