 ...
 * GET / PUT / DELETE for a single service category
 */
...
async function load(prisma, user, id) {
  ...
  const cat = await prisma.wehowareServiceCategory.findFirst({ where: { id, clientId } });
...
export const GET = withAuth(async (request, { params }) => {
  try {
    ...
    console.error("[GET /api/v1/services/categories/[id]]", err);
...
export const PUT = withAuth(async (request, { params }) => {
  try {
    ...
    const updated = await prisma.wehowareServiceCategory.update({ where: { id }, data });
...
export const DELETE = withAuth(async (request, { params }) => {
  try {
    ...
    const auth = await load(prisma, user, id);
    if (auth.status !== 200) return NextResponse.json(auth.body, { status: auth.status });
    await prisma.wehowareServiceCategory.delete({ where: { id } });
